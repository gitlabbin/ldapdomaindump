...
class Utils(object):
    ...
    def get_group_object(groupo, domain):
        return {
            ...
            "type": "group",
            "principal": "%s@%s"
            % (groupo["attributes"]["sAMAccountName"][0].upper(), domain.upper()),
            "memberOf": groupo["attributes"]["memberOf"]
            if "memberOf" in groupo["attributes"]
            else [],
        }
...
class BloodHoundConverter(object):
    ...
    def build_mappings(self):
        ...
        for file in self.group_files:
            ...
            for group in data:
                groupattrs = Utils.get_group_object(
                    group,
                ...
                for membergroup in group["memberOf"]:
                    try:
                        outfile.write(
                            "%s,%s,%s\n"
                            % (
                                self.groups_by_dn[membergroup]["principal"],
                                group["principal"],
                                "group",
                            )
                        )
                        ...
                        logger.warning(
                            "Unknown group %s. Not found in groups cache!", membergroup
                        )

    def write_trusts(self):
        direction_map = {
            flag: meaning.capitalize() for meaning, flag in trust_directions.items()
        }
    ...
    def write_entry_memberships(self, entry, outfile, entry_type="user"):
        ...
        if entry_type == "user":
            principal = "%s@%s" % (
                entry["attributes"]["sAMAccountName"][0].upper(),
                domain.upper(),
            )
        else:
            principal = "%s.%s" % (
                entry["attributes"]["sAMAccountName"][0][:-1].upper(),
                domain.upper(),
            )
        if "memberOf" in entry["attributes"]:
            for group in entry["attributes"]["memberOf"]:
                try:
                    ...
                    logger.warning(
                        "Unknown group %s. Not found in groups cache!", group
                    )
        # Now process primary group id
        dsid = entry["attributes"]["objectSid"][0].rsplit("-", 1)[0]
        try:
            rgroup = self.groups_by_sid[
                "%s-%d" % (dsid, entry["attributes"]["primaryGroupID"][0])
            ]
            outfile.write("%s,%s,%s\n" % (rgroup["principal"], principal, entry_type))
        except KeyError:
            logger.warning(
                "Unknown rid %d. Not found in groups cache!",
...
def ldd2bloodhound():
    ...
    parser.add_argument(
        "-d", "--debug", action="store_true", help="Enable debug logger"
    )
    ...
    if len(converter.group_files) == 0:
        logger.error(
            "No domain_groups.json files were specified. Need at least one to perform conversion."
        )
        return
    if len(converter.user_files) == 0:
        logger.error(
            "No domain_users.json files were specified. Need at least one to perform conversion."
        )
        return
