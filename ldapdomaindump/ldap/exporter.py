...
class ReportWriter(object):
    def __init__(self, config):
        ...
        if self.config.lookuphostnames:
            self.computerattributes = ['cn', 'sAMAccountName', 'dNSHostName', 'IPv4', 'operatingSystem',
                                       'operatingSystemServicePack', 'operatingSystemVersion', 'lastLogon',
                                       'userAccountControl', 'whenCreated', 'objectSid', 'description']
        else:
            self.computerattributes = CUST_COMPUTERATTRIBUTES
        self.userattributes = ['cn', 'name', 'sAMAccountName', 'memberOf', 'primaryGroupId', 'whenCreated',
                               'whenChanged', 'lastLogon', 'userAccountControl', 'pwdLastSet', 'objectSid',
                               'description']
        # In grouped view, don't include the memberOf property to reduce output size
        self.userattributes_grouped = ['cn', 'name', 'sAMAccountName', 'whenCreated', 'whenChanged', 'lastLogon',
                                       'userAccountControl', 'pwdLastSet', 'objectSid', 'description']
        self.groupattributes = ['cn', 'sAMAccountName', 'memberOf', 'description', 'whenCreated', 'whenChanged',
                                'objectSid']
        self.policyattributes = ['distinguishedName', 'lockOutObservationWindow', 'lockoutDuration', 'lockoutThreshold',
                                 'maxPwdAge', 'minPwdAge', 'minPwdLength', 'pwdHistoryLength', 'pwdProperties',
                                 'ms-DS-MachineAccountQuota']
        self.trustattributes = ['cn', 'flatName', 'securityIdentifier', 'trustAttributes', 'trustDirection',
                                'trustType']
    ...
    def generateHtmlTable(self, listable, attributes, header='', firstTable=True, specialGroupsFormat=False):
        ...
        # Table header
        if header != '':
            of.append('<thead><tr><td colspan="%d" id="cn_%s">%s</td></tr></thead>' % (
    ...
    def writeHtmlFile(self, rel_outfile, body, genfunc=None, genargs=None, closeTable=True):
        ...
        with codecs.open(outfile, 'w', 'utf8') as of:
            ...
            try:
                ...
                logging.warning('style.css not found in package directory, styling will be skipped')
            of.write('</head><body>')
    ...
    def formatCnWithGroupLink(self, cn):
        return 'Group: <a href="#cn_%s" title="%s">%s</a>' % (
            self.formatId(cn), self.htmlescape(cn), self.htmlescape(cn))
    ...
    def formatGroupsHtml(self, grouplist):
        ...
        for group in grouplist:
            ...
            outcache.append('<a href="%s.html#cn_%s" title="%s">%s</a>' % (
                self.config.users_by_group, quote_plus(self.formatId(cn)), self.htmlescape(group), self.htmlescape(cn)))
    ...
    def generateComputersByOsReport(self, dd):
        ...
        if self.config.outputhtml:
            ...
            self.writeHtmlFile('%s.html' % self.config.computers_by_os, None, genfunc=self.generateGroupedHtmlTables,
                               genargs=(grouped, self.computerattributes))
        if self.config.outputjson and self.config.groupedjson:
            self.writeJsonFile('%s.json' % self.config.computers_by_os, None, genfunc=self.generateJsonGroupedList,
                               genargs=(grouped,))
    ...
    def generateUsersByGroupReport(self, dd):
        ...
        if self.config.outputhtml:
            # Use the generator approach to save memory
            self.writeHtmlFile('%s.html' % self.config.users_by_group, None, genfunc=self.generateGroupedHtmlTables,
                               genargs=(grouped, self.userattributes_grouped))
        if self.config.outputjson and self.config.groupedjson:
            self.writeJsonFile('%s.json' % self.config.users_by_group, None, genfunc=self.generateJsonGroupedList,
                               genargs=(grouped,))
