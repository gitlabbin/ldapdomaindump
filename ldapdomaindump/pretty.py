...
class PrettyOuput(object):
    ...
    """
        ...
        print(
            "\t[+] Minimum password length: ", data[0]["attributes"]["instanceType"][0]
        )
        print(
            "\t[+] Password history length:",
            data[0]["attributes"]["pwdHistoryLength"][0],
        )
        ...
        print(
            ...
            self.convert(data[0]["attributes"]["maxPwdAge"][0]),
        )
        print(
            "\t[+] Minimum password age:",
            self.convert(data[0]["attributes"]["minPwdAge"][0]),
        )
        print(
            "\t[+] Reset Account Lockout Counter:",
            self.convert(data[0]["attributes"]["lockoutDuration"][0]),
        )
        print(
            "\t[+] Account Lockout Threshold:",
            data[0]["attributes"]["lockoutThreshold"][0],
        )
        print(
            "\t[+] Forced Log off Time:",
            self.convert(data[0]["attributes"]["forceLogoff"][0]),
        )
    ...
    """
        ...
        for user in users:
            desc = (
                user["attributes"].get("description")[0]
                if user["attributes"].get("description")
                else "(null)"
            )
            print(
                "Account: "
                + dc
                + "\\"
                + user["attributes"]["sAMAccountName"][0]
                + "\tName: "
...
def main():
    parser = argparse.ArgumentParser(
        description="LDAPDomainDump to pretty output like enum4linux."
    )
